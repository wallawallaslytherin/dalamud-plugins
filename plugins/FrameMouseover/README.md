...
Updates are tested in the owner's local developer copy and batched for release.
Publishing any update requires the owner's explicit instruction, separately from
in-game test confirmation. See the [maintainer release policy](../../README.md#maintainer-release-policy).

Current UI, commands and documentation use Hovercast. Follow the
[maintainer naming policy](../../README.md#maintainer-naming-policy) when changing
the product name; do not leave obsolete command aliases or mixed branding.
